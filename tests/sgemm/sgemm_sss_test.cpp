...
//
// ============================================================================
// sgemm_sss_test: F32 x F32 -> F32 Matrix Multiplication Benchmark
// ============================================================================
//
// PURPOSE:
//   This test benchmarks large matrix multiplication (SGEMM) performance,
//   comparing llamafile's optimized CPU kernels against llama.cpp's fallback.
//
// OPERATION:
...
//      - Performs row-by-row dot products using SIMD (SSE/AVX on x86, NEON on ARM)
//      - Single-precision accumulation
//
//   2. llamafile_sgemm_openmp (optimized)
//      - llamafile's optimized tinyblas CPU kernels
//      - Uses architecture-specific implementations:
//        * ARM: NEON with optional FP16/dotprod extensions (arm80, arm82)
//        * x86: AVX, AVX2, AVX-512, or Zen4-specific optimizations
//      - Employs cache-friendly tiling and register blocking
...
//     - Lower is better
//     - Reported as average over multiple iterations
//
//   Accuracy (ULP = Units in Last Place):
//     - Measures floating-point precision difference between implementations
//     - ULP is the gap between two consecutive floating-point numbers
//     - "ulp average": Mean ULP difference across all output elements
//     - "ulp worst": Maximum ULP difference for any single element
//     - Differences arise from different accumulation order and precision
//     - Typical values: 100-1000 ULP average is normal for large k
//
// TEST PARAMETERS:
//   - m=510, n=513, k=260000 (large k simulates LLM weight matrix dimensions)
//   - Non-power-of-2 dimensions test edge case handling
//   - Matrices padded to 16-element boundaries (lda, ldb, ldc)
//
...
void print_test_info() {
    printf("============================================================================\n");
    printf("sgemm_sss_test: F32 x F32 -> F32 Matrix Multiplication Benchmark\n");
    printf("============================================================================\n");
    printf("\n");
    printf("OPERATION:\n");
    printf("  C = A^T * B  (m x n result from k x m and k x n inputs)\n");
    printf("  This simulates the matrix multiplication in LLM inference.\n");
    printf("\n");
    printf("IMPLEMENTATIONS:\n");
    printf("  ggmlBLAS::sgemm        - llama.cpp's fallback using ggml_vec_dot_f32()\n");
    printf("                           This is what runs when llamafile_sgemm returns false.\n");
    ...
    printf("                ULP differences are normal due to accumulation order.\n");
    printf("\n");
}
